...
class Alias:
    ...
    async def _add_alias(self, ctx, command, *, to_execute):
        """Ajouter un alias à une commande

           Exemple: !alias add test flip @Acrown"""
        server = ctx.message.server
        command = command.lower()
        if len(command.split(" ")) != 1:
            await self.bot.say("Les alias ne peuvent pas contenir plusieurs mots. Circulez.")
            return
        if self.part_of_existing_command(command, server.id):
            await self.bot.say('Désolé mais une commande similaire existe déjà, ça serait bête de la casser non ?')
            return
        ...
        if command not in self.bot.commands:
            ...
            await self.bot.say("Impossible d'ajouter '{}' parce qu'elle... existe déjà en fait.".format(command))

    @alias.command(name="help", pass_context=True, no_pm=True)
    async def _help_alias(self, ctx, command):
        """Donne un maximum d'informations sur les alias, du mieux qu'il peut."""
        server = ctx.message.server
    ...
    async def _show_alias(self, ctx, command):
        """Montre quelle commande l'alias visé exécute."""
        server = ctx.message.server
        if server.id in self.aliases:
            server_aliases = self.aliases[server.id]
            if command in server_aliases:
    ...
    async def _alias_list(self, ctx):
        """Liste les alias disponibles sur ce serveur en MP"""
        server = ctx.message.server
        if server.id in self.aliases:
            message = "```Liste des Alias:\n"
            for alias in sorted(self.aliases[server.id]):
                if len(message) + len(alias) + 3 > 2000:
                    await self.bot.whisper(message)
                    message = "```\n"
                message += "\t{}\n".format(alias)
            if message != "```Liste des Alias:\n":
                message += "```"
                await self.bot.whisper(message)
            else:
                await self.bot.say("Aucun alias n'existe sur ce serveur.")
