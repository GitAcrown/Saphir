...
INTRO = ("==========================\n"
         "Saphir Bot Discord - Launcher EN\n"
         "==========================\n")
...
def parse_cli_arguments():
    parser = argparse.ArgumentParser(description="Saphir - Discord Bot's launcher")
...
def update_red():
    try:
        ...
        return
    if code == 0:
        print("\nSaphir has been updated")
    else:
        print("\nSaphir could not update properly. If this is caused by edits "
...
def reset_red(reqs=False, data=False, cogs=False, git_reset=False):
    ...
    if git_reset:
        ...
        if code == 0:
            print("Saphir has been restored to the last local commit.")
        else:
...
def update_menu():
    ...
    while True:
        ...
        print("Update:\n")
        print("Saphir:")
        print("1. Update Saphir + requirements (recommended)")
        print("2. Update Saphir")
...
def maintenance_menu():
    ...
    while True:
        print(INTRO)
        print("Maintenance:\n")
        print("1. Repair Saphir (discards code changes, keeps data intact)")
        ...
        if choice == "1":
            ...
            print("Are you sure? This will wipe ALL your Saphir's installation "
...
def run_red(autorestart):
    ...
    if verify_requirements() is None:
        print("You don't have the requirements to start Saphir. "
    ...
    while True:
        try:
            ...
            if code == 0:
                ...
                print("Restarting Saphir...")
                continue
            else:
                if not autorestart:
                    break

    print("Saphir has been terminated. Exit code: %d" % code)
...
def create_fast_start_scripts():
    """Creates scripts for fast boot of Saphir without going
...
def main():
    ...
    if IS_WINDOWS:
        os.system("TITLE Saphir Discord Bot - Launcher")
    clear_screen()
    ...
    while True:
        ...
        if not is_git_installation:
            print("WARNING: It doesn't look like Saphir has been "
        ...
        print("1. Run Saphir /w autorestart in case of issues")
        print("2. Run Saphir")
        print("3. Update")
...
if __name__ == '__main__':
    ...
    os.chdir(dirname)
    if not PYTHON_OK:
        print("Saphir needs Python 3.5 or superior. Install the required "
              "version.\nPress enter to continue.")
        if INTERACTIVE_MODE:
            wait()
        exit(1)
    if pip is None:
        print("Saphir cannot work without the pip module. Please make sure to "
